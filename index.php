...
if ($result->num_rows > 0) {
	...
	while($row = $result->fetch_assoc())
	{
		...
		if($row["Verified"] == 0)
		{
			$linkText = "Verify Highlight Location";
		}
		else
		{
			$linkText = "Update Highlight Location";
			if($row["YouTubeLink"] == "")
			{	
				$videoEmbed = "<br><a type='button' class='btn btn-success btn-sm' href='./index.php?gameId=$gameId&highlightId=$highlightId&action=upload'>Send to YouTube</a><br>
				<small>NOTE: if Send to YouTube is failing, we likely hit our daily quota. Try again tomorrow.</small>";
			}
		...
		$videoCol = "<h5><a href='#player' id='highlight$highlightId' onclick='verifyHighlight($highlightId, $gameId, \"$highlightName\", $startTime, $endTime, \"$highlightType\", \"$highlightSubType\", \"$goalTags\")' class=''>$linkText: " . gmdate("H:i:s", $startTime) . "</a></h5>";
		$videoCol .= $videoEmbed;
		$highlightsTable .= "<tr><th scope='row'><h5><a href='#' onclick='seek($startTime);' class=''>" . gmdate("H:i:s", $startTime) . "</a></h5></th><td>" . $row["Type"] . "</td><td>" . $nameCol . "</td><td>" . $videoCol . "</td></tr>";
		
		if($highlightId == $selectedHighlightId)
		{
			$onload = "windowLocation(\"#highlight$highlightId\")";
