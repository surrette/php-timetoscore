...
if(!empty($_GET['highlightId'])) 
{
	...
	if ($conn->query("UPDATE highlights set YouTubeLink='PENDING' where HighlightId='$highlightId'") === TRUE) {
...
if ($result->num_rows > 0) {
	...
	while($row = $result->fetch_assoc())
	{
		...
		if($row["Verified"] == 0)
		{
			$videoEmbed = "<a href='#player' onclick='verifyHighlight($highlightId, \"$highlightName\", $startTime, $endTime, \"$highlightType\", \"$highlightSubType\", \"$goalTags\")' class=''>Verify Highlight</a>";
		}
		else
		{
			$videoEmbed = "<a href='#player' onclick='verifyHighlight($highlightId, \"$highlightName\", $startTime, $endTime, \"$highlightType\", \"$highlightSubType\", \"$goalTags\")' class=''>Update Highlight</a>";
			if($row["YouTubeLink"] == "")
			{
				$videoEmbed .= "<br><a type='button' class='btn btn-success btn-sm' href='./index.php?gameId=" . $row["GameId"] . "&highlightId=$highlightId'>Send to YouTube</button><br>";
			}
		...
		<form action="index.php?gameId=<?php echo $gameId; ?>" method="POST">
		  <div class="form-group row">
			...
			<div class="col-sm-2 text-right">
			  <button type="button" class="btn btn-secondary" onclick="setStart()">Set Start</button>
			  <button type="button" class="btn btn-secondary" onclick="seekStart()">Jump To</button>
			</div>
			<div class="col-sm-2 text-center">
				<button hidden="hidden" type="submit" value="update" name="update", id="update" class="btn btn-success">Update Highlight</button>
				<button type="submit" value="new" name="new", id="new" class="btn btn-primary">New Highlight</button>
			</div>
			<div class="col-sm-2 text-left">
			  <button type="button" class="btn btn-secondary" onclick="setEnd()">Set End</button>
			  <button type="button" class="btn btn-secondary" onclick="seekEnd()">Jump To</button>
			</div>
			<div class="col-sm-2">
	...
	<script>
	 ...
	 // 2. This code loads the IFrame Player API code asynchronously.
	  ...
	  function onYouTubeIframeAPIReady() {
		  <?php if($gameRow["Surface"] == "San Jose Center")
			{?>
				var h = '1024';
				var w = '2560';
			<?php 
